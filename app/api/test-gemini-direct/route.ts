...
export async function POST(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          details:
            "Отсутствуют переменные окружения GEMINI_API_KEY или GEMINI_API_URL",
        },
        { status: 500 },
      );
    }
    ...
    const testPrompt =
      prompt || "Привет! Ответь кратко на русском языке, что ты можешь делать.";
    ...
    if (!response.ok) {
      ...
      if (
        response.status === 400 &&
        errorData.error?.message?.includes("User location is not supported")
      ) {
        userMessage =
          "Google AI Studio API недоступен в вашем регионе. Используйте VPN или альтернативные AI сервисы.";
      } else if (
        response.status === 404 &&
        errorData.error?.message?.includes("not found")
      ) {
        userMessage =
          "Указанная модель Gemini не найдена. Проверьте конфигурацию модели.";
