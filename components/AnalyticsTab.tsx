...
import { Loader2, Activity, CheckCircle, XCircle, Clock, FilterX } from 'lucide-react'
...
export default function AnalyticsTab() {
  ...
  const performanceRows = useMemo(() => {
    ...
    filteredHistory.forEach(item => {
      ...
      if (!map.has(key)) {
        map.set(key, { total: 0, success: 0, responseTotal: 0, responseCount: 0 })
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {selectedService === 'all' ? 'Сравнение сервисов' : 'Сравнение эндпоинтов'}
          </CardTitle>
        </CardHeader>
        <CardContent>
