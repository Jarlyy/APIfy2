...
import { CheckCircle, Clock, Eye, EyeOff, Play, XCircle } from "lucide-react";
...
export default function OpenApiImport() {
  ...
  const [executableEndpoints, setExecutableEndpoints] = useState<
    ExecutableEndpoint[]
  >([]);
  ...
  const createExecutableEndpoints = (
  ...
  const handleUrlImport = async () => {
    ...
    try {
      ...
      const executable = createExecutableEndpoints(
        parsedEndpoints,
        specData,
        specUrl,
      );
      ...
      if (specData.servers?.[0]?.url) {
        console.log(
          "Устанавливаем базовый URL из спецификации:",
          specData.servers[0].url,
        );
    ...
    try {
      ...
      if (corsProxyEnabled) {
        ...
        try {
          ...
          proxyResult = {
            data: `Ошибка парсинга: ${parseError}`,
            status: response.status,
          };
        }
      }
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <span className="font-medium">Эндпоинтов:</span>{" "}
                {endpoints.length}
              </div>
            </div>
