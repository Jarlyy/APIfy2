...
export function UnifiedApiTester({ userId, testData }: UnifiedApiTesterProps) {
  ...
  const [corsProxyEnabled, setCorsProxyEnabledState] = useState(
    getCorsProxyEnabled(),
  );
  ...
  const generateExecutableTests = async () => {
    ...
    try {
      ...
      try {
        ...
        if (typeof data.result === "object") {
          ...
          if (jsonMatch) {
            ...
            if (lastQuoteIndex > -1) {
              ...
              if (
                ...
                if (colonIndex > lastQuoteIndex) {
                  ...
                  const afterColon = jsonString
                    .substring(colonIndex + 1)
                    .trim();
                  if (
                    afterColon.startsWith('"') &&
                    !afterColon.substring(1).includes('"')
                  ) {
            ...
            try {
              ...
              console.error(
                "JSON parsing failed, attempting to extract valid objects...",
              );
              ...
              if (objectMatches && objectMatches.length > 0) {
                ...
                if (testsData.length > 0) {
                  ...
                  throw new Error(
                    "Не удалось извлечь валидные тесты из ответа",
                  );
                }
              } else {
                throw new Error(
                  `Ошибка парсинга JSON: ${parseError instanceof Error ? parseError.message : "Неизвестная ошибка"}`,
                );
              }
            }
          } else {
            ...
            try {
              ...
              console.error(
                "Direct JSON parsing failed, attempting to fix truncated JSON...",
              );
              ...
              if (!fixedJson.endsWith("]")) {
                ...
                const lastCompleteObjectMatch =
                  fixedJson.match(/.*(\{[^{}]*\})/);
                if (lastCompleteObjectMatch) {
                  const lastCompleteIndex =
                    fixedJson.lastIndexOf(lastCompleteObjectMatch[1]) +
                    lastCompleteObjectMatch[1].length;
                  ...
                  const objectMatches = fixedJson.match(
                    /\{[^{}]*(?:\{[^{}]*\}[^{}]*)*\}/g,
                  );
                  if (objectMatches && objectMatches.length > 0) {
              ...
              try {
                ...
                console.log(
                  "Successfully fixed truncated JSON, recovered",
        ...
        alert(
          `Не удалось распарсить тесты от AI.\n\n${e instanceof Error ? e.message : "Неизвестная ошибка"}\n\nПроверьте консоль для деталей.`,
        );
        setGeneratedTests([]);
      }
    } catch (error) {
      ...
      alert(
        `Ошибка при генерации тестов: ${error instanceof Error ? error.message : "Неизвестная ошибка"}`,
      );
    } finally {
      setAiLoading(false);
    }
      ...
      if (corsProxyEnabled) {
        ...
        if (proxyType === "local") {
          ...
          const result: TestResult = {
            id: test.id,
            status:
              proxyResult.status === test.expected_status ? "success" : "error",
  ...
  const runManualTest = async () => {
    ...
    try {
      ...
      if (manualTest.authType === "bearer" && manualTest.bearerToken) {
        parsedHeaders = {
          ...parsedHeaders,
          Authorization: `Bearer ${manualTest.bearerToken}`,
        };
      } else if (manualTest.authType === "api-key" && manualTest.apiKey) {
        parsedHeaders = {
          ...parsedHeaders,
          [manualTest.apiKeyHeader]: manualTest.apiKey,
        };
      } else if (
        manualTest.authType === "basic" &&
        manualTest.basicUsername &&
        manualTest.basicPassword
      ) {
        const credentials = btoa(
          `${manualTest.basicUsername}:${manualTest.basicPassword}`,
        );
        parsedHeaders = {
          ...parsedHeaders,
      ...
      if (corsProxyEnabled) {
        ...
        if (proxyType === "local") {
          ...
          const proxyResponse = await fetch(proxyUrl, {
            ...
            body:
              manualTest.method !== "GET" && manualTest.body
                ? manualTest.body
                : undefined,
          });
  ...
  const saveToFavorites = async () => {
    if (!manualTest.url || !manualTest.method || !manualTest.serviceName) {
      showNotification(
        "Заполните название сервиса, URL и выберите метод для сохранения в избранное",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      if (manualTest.authType === "bearer" && manualTest.bearerToken) {
        ...
        authData = {
          ...
          header: manualTest.apiKeyHeader,
        };
      } else if (
        manualTest.authType === "basic" &&
        manualTest.basicUsername &&
        ...
        authData = {
          username: manualTest.basicUsername,
          password: manualTest.basicPassword,
        };
      }
      ...
      if (result.success) {
        ...
        showNotification(
          `Ошибка добавления в избранное: ${result.error}`,
          "error",
        );
      }
    } catch (error) {
      ...
      <Card>
        ...
        <CardContent>
          ...
          >
            ...
            <div
              className={`space-y-4 mt-4 ${activeTab !== "ai-tests" ? "hidden" : ""}`}
            >
              {/* AI Generation Section */}
              <div className="space-y-3 pb-4 border-b">
                <div className="space-y-2">
                  <label className="text-sm font-medium">
                    Создать тесты с помощью ИИ
                  </label>
                  <div className="flex gap-2">
                    <Input
                      ...
                      onKeyDown={(e) =>
                        e.key === "Enter" && generateExecutableTests()
                      }
                ...
                <AIProviderSelector onProviderChange={handleProviderChange} />
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium">
                      {generatedTests.length} готовых тестов
                    </span>
                    <Button
                      onClick={runAllTests}
                      disabled={runningAll}
                          ...
                          <CardContent className="p-3">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-3 flex-1 min-w-0">
                                <span
                                  className={`px-2 py-1 rounded text-xs font-medium ${getMethodColor(test.method)}`}
                                >
                                  {test.method}
                                </span>
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <Badge
                                      variant="outline"
                                      className={`${getCategoryColor(test.category)} text-xs px-1 py-0`}
                                    >
                                      {test.category}
                                    </Badge>
                                  </div>
            ...
            {/* Manual Test Tab */}
            <div
              className={`space-y-4 mt-4 ${activeTab !== "manual" ? "hidden" : ""}`}
            >
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    disabled={
                      savingToFavorites ||
                      !manualTest.url ||
                      !manualTest.serviceName ||
                      isCurrentTestFavorite
                    }
                  ...
                  >
                    {savingToFavorites ? (
                      ...
                      <Heart
                        className={`h-4 w-4 ${isCurrentTestFavorite ? "fill-current text-red-500" : ""}`}
                      />
                    )}
                  </Button>
                </div>
                  ...
                  <Card>
                    <CardHeader className="pb-3">
                      <CardTitle className="text-base flex items-center gap-2">
                        {manualResult.status >= 200 &&
                        manualResult.status < 300 ? (
                          <CheckCircle className="h-4 w-4 text-green-500" />
                        ) : (
                          <XCircle className="h-4 w-4 text-red-500" />
                        )}
                        Результат: {manualResult.status}{" "}
                        {manualResult.statusText}
                      </CardTitle>
                      <CardDescription>
