import { type TestHistoryItem, saveTestToHistory } from "./test-history";
    ...
    for (const localTest of allLocalTests) {
      try {
        ...
        const historyItem: TestHistoryItem = {
          service_name:
            localTest.serviceName || localTest.service_name || "Migrated Test",
          ...
          response_time:
            localTest.result?.responseTime || localTest.response_time,
          error_message: localTest.result?.error || localTest.error_message,
          test_status:
            localTest.result?.status >= 200 && localTest.result?.status < 300
              ? "success"
              : "error",
